...
lemma not_recursive_cspaces_time_independent_simple[simp]:
  ...
  "not_recursive_cspaces (s \<lparr> ready_queues := q \<rparr>) = not_recursive_cspaces s"
  "not_recursive_cspaces (s \<lparr> scheduler_action:= sa \<rparr>) = not_recursive_cspaces s"
  "not_recursive_cspaces (s \<lparr> domain_time:= t \<rparr>) = not_recursive_cspaces s"
  "not_recursive_cspaces (s \<lparr> reprogram_timer:= f \<rparr>) = not_recursive_cspaces s"
  by (simp add: not_recursive_cspaces_def)+

context CNodeInv_AI begin

lemma fffffff[wp]:
  "\<lbrace>\<lambda>s. if b then Q () s else R () s\<rbrace>
   if b then throwError() else returnOk() 
  \<lbrace>R\<rbrace>, \<lbrace>Q\<rbrace>"
  apply (clarsimp simp: validE_def throwError_def returnOk_def)
  apply (auto)
  apply wpsimp
  apply wpsimp
  done
...
lemma preemption_point_not_recursive_cspaces:
  "preemption_point \<lbrace>\<lambda>s. P (not_recursive_cspaces s)\<rbrace>"
  unfolding preemption_point_def
  apply (wpsimp wp: OR_choiceE_weak_wp alternative_valid hoare_drop_imp comb: fffffff)
  
  apply (clarsimp simp: validE_def throwError_def)
  apply (rule hoare_seq_ext)+
  apply (wpsimp wp: hoare_drop_imp)+

  apply (wpsimp wp: OR_choiceE_weak_wp alternative_valid hoare_drop_imp comb: fffffff)
  apply (clarsimp simp: validE_R_def validE_def throwError_def)

  apply (rule hoare_seq_ext)+
  apply (wpsimp wp: hoare_drop_imp)+
  done

lemma preemption_point_caps_of_state:
...
lemma rec_del_termination:
  ...
  apply (rule rec_del.termination[OF rec_del_recset_wf])
     ...
     apply (simp add: rec_del_recset_def wf_sum_def 
                      rdcall_finalise_ord_lift_def
                      mlex_prod_def)
...
lemma cap_revoke_typ_at:
  ...
  by (wpsimp wp: cap_delete_typ_at cap_revoke_preservation irq_state_independent_AI preemption_point_inv)
